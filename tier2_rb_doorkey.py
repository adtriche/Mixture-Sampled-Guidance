...
def train_rb(size=13, total_steps=60000, max_steps=2000, rollout=512, epochs=4, minibatch=128,
             ...
             lam=1.0, rnd_lr=1e-4, device="cpu", seed=0, log_every=2000, runs_dir="runs", tag=""):
    torch.manual_seed(seed); np.random.seed(seed)
    config = dict(arm="rb", size=size, total_steps=total_steps, max_steps=max_steps, lam=lam,
    ...
    arm_dir = os.path.join(runs_dir, "tier2", "rb")
    os.makedirs(arm_dir, exist_ok=True)
    stem = run_stem("rb", size, lam, seed, tag)
    ...
    env = make_doorkey_env(size=size, max_steps=max_steps, gamma=gamma)
    ...
    while global_step < total_steps:
        ...
        rew_buf = [re + lam * ri for re, ri in zip(rew_ext_buf, rew_int_buf)]
    ...
    for lam in args.lam:
        for seed in args.seed:
            ...
            train_rb(size=args.size, total_steps=args.steps, max_steps=args.max_steps,
                     ...
                     lr_anneal_floor=args.lr_anneal_floor, lam=lam, rnd_lr=args.rnd_lr,
